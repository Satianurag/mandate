...
export function buildAgentReport(run:AgentRun,events:AgentEvent[],payments:AgentPaymentView[]){
  ...
  const usages=events.filter(e=>e.kind==='model_usage').map(e=>e.data as Record<string,unknown>);
  ...
  return {version:1,title:run.agent.name,goal:run.goal,status:run.state,result:run.result,error:run.error,runId:run.id,generatedAt:run.updatedAt,
    ...
    boundary:'Payments are testnet USDC. Source data may be read-only mainnet analytics. The model chooses actions; only the trusted broker can authorize a bounded payment. Vertex inference is billed separately.'};
}
...
export function agentReportToMarkdown(doc:ReturnType<typeof buildAgentReport>):string{
 ...
 parts.push('','## Payment receipts','','| Request | Network | State | Amount (test USDC) | Chain proof |','| --- | --- | --- | --- | --- |');
