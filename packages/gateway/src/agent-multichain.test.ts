...
function fixture(t:{after:(f:()=>void)=>void},loseResponse=false, loseCache=false){
 ...
 const authority:ExactAgentAuthority={id:'multi-chain-fixture',network:'eip155:8453',asset:EXACT_USDC['eip155:8453'],payerAddress:account.address,spendingAddress:account.address,expiresAt:Date.now()+3600000,ceilingBaseUnits:'3000',perCallBaseUnits:'2000',windowBaseUnits:'3000',windowMs:60000,
 tools:[{id:'graph-protocol',origin:'https://tools.example.org',pathname:'/tools/graph-protocol',payTo:'0x0000000000000000000000000000000000000002'}]};
 const config={protocols:[{id:'protocol',label:'Controlled fixture',endpoint:'https://gateway.thegraph.com/api/x402/subgraphs/id/GqzP4Xaehti8KSfQmv3ZctFSjnSUYZ4En5NRsiTbvZpz',queries:[{id:'overview',description:'Fixture',query:'{ factories(first:1) { id } }'}]}],endpoints:{'graph-protocol':'https://tools.example.org/tools/graph-protocol','hedera-analysis':scope.endpoint}};
 ...
 const offerFor=(hedera:boolean):PaymentRequirements=>hedera?{scheme:'exact',network:'hedera:testnet',asset:'0.0.429274',amount:'2000',payTo:scope.payTo,maxTimeoutSeconds:120,extra:{feePayer:scope.feePayer}}:{scheme:'exact',network:authority.network,asset:authority.asset,amount:'1000',payTo:authority.tools[0]!.payTo,maxTimeoutSeconds:120,extra:{name:'USD Coin',version:'2',assetTransferMethod:'eip3009'}};
 const executor=new ExactAgentExecutor({authority,journal,tools:createAgentTools(config),receiptLookupUrls:Object.values(config.endpoints),receiptLocator:{checkpoint:async()=>"100",find:async(_payload,_offer,_payer,block)=>{assert.equal(block,"100");return `0x${"aa".repeat(32)}`;}},
