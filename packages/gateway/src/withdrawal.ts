...
export function assessWithdrawal(
  ...
  assertTestnetChain(BASE_SEPOLIA_CHAIN_ID);
  const channelId = computeChannelId(config, BASE_SEPOLIA_CHAIN_ID);
  if (snapshot.network !== "eip155:84532") throw new Error("Withdrawal snapshot is from another network");
...
export function assertPreparedWithdrawalCurrent(input: {
  ...
  if (plan.network !== "eip155:84532" || plan.chainId !== BASE_SEPOLIA_CHAIN_ID || getAddress(plan.contract) !== getAddress(BATCH_SETTLEMENT_ADDRESS)) throw new Error("Stored withdrawal plan is not Base Sepolia x402 escrow");
  ...
  if (input.currentNativeBalanceWei < BigInt(plan.maxGasCostWei)) throw new Error("Payer no longer has enough Base Sepolia ETH for the reviewed maximum gas cost");
...
export async function readPreparedWithdrawalCurrent(input: {
  ...
  if (await publicClient.getChainId() !== BASE_SEPOLIA_CHAIN_ID) throw new Error("Withdrawal RPC is not Base Sepolia");
...
export type ConfirmedWithdrawal =
  | { kind: "initiate"; transaction: Hex; amountBaseUnits: string; readyAt: number; before: ChannelSnapshot; after: ChannelSnapshot; network: "eip155:84532" }
  | { kind: "finalize"; transaction: Hex; returnedBaseUnits: string; before: ChannelSnapshot; after: ChannelSnapshot; network: "eip155:84532" };
...
export async function confirmWithdrawalTransaction(input: {
  ...
  const publicClient = createPublicClient({ transport: http(input.cfg.rpcUrl, { timeout: 15_000, retryCount: 1 }) });
  if (await publicClient.getChainId() !== BASE_SEPOLIA_CHAIN_ID) throw new Error("Withdrawal confirmation RPC is not Base Sepolia");
  ...
  if (input.plan.kind === "initiate") {
    ...
    return { kind: "initiate", transaction: input.transactionHash, amountBaseUnits: input.plan.amountBaseUnits, readyAt, before, after, network: "eip155:84532" };
  }
  ...
  return { kind: "finalize", transaction: input.transactionHash, returnedBaseUnits: input.plan.amountBaseUnits, before, after, network: "eip155:84532" };
}
...
export async function broadcastSignedWithdrawal(input: {
  ...
  const publicClient = createPublicClient({ transport: http(input.cfg.rpcUrl, { timeout: 15_000, retryCount: 1 }) });
  if (await publicClient.getChainId() !== BASE_SEPOLIA_CHAIN_ID) throw new Error("Withdrawal broadcast RPC is not Base Sepolia");
...
export async function prepareWithdrawalTransaction(input: {
  ...
  if (payerNativeBalance < maxGasCostWei) throw new Error("Ledger payer has insufficient Base Sepolia ETH for the reviewed withdrawal transaction");
...
export function unsignedWithdrawalBytes(plan: PreparedWithdrawalTransaction): Uint8Array {
  ...
  if (parsed.chainId !== BASE_SEPOLIA_CHAIN_ID || !parsed.to || getAddress(parsed.to) !== getAddress(BATCH_SETTLEMENT_ADDRESS) || parsed.data !== plan.callData || (parsed.value ?? 0n) !== 0n) {
    throw new Error("Stored withdrawal plan is not the reviewed Base Sepolia escrow transaction");
  }
