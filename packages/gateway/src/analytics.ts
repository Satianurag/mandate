...
export interface AnalyticsPayload {
  ok: true;
  query: string;
  source: { provider: "the-graph"; chain: string; subgraphId: string };
  rows: Agent0Row[];
}
...
export async function liveAnalyticsBody(
  ...
  if (deps.source) {
    const discovered = subgraphs[deps.source.chain];
    if (!discovered) throw new Error(`Selected Agent0 source ${deps.source.chain} is not available from live discovery.`);
    if (discovered !== deps.source.deployment) throw new Error("Selected Agent0 deployment no longer matches live discovery; review a new mandate instead of switching sources silently.");
    const data = await queryAgent0(discovered, apiKey, query, undefined, deps.fetchFn);
    ...
    return { ...analyticsPayload(query, { provider: "the-graph", chain: deps.source.chain, subgraphId: discovered }, (raw.agents ?? []).map(toRow), paid),
  ...
  for (const chain of preferred) {
    ...
    try {
      ...
      return {...analyticsPayload(query, {provider:"the-graph",chain,subgraphId}, (raw.agents ?? []).map(toRow), paid),
...
export function analyticsPayload(
  ...
  return { ok: true, query, source: { provider: "the-graph", ...source }, observedAt: new Date().toISOString(),
