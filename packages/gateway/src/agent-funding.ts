...
export class AgentFundingController {
  ...
  constructor(options:AgentFundingOptions){
    ...
    if(options.authority.network!=='eip155:8453')throw new Error('Agent funding is restricted to Base mainnet');
  ...
  review(){
    ...
    return {authority:structuredClone(a),effectiveCeilingBaseUnits,confirmedIncreaseBaseUnits:journal.confirmedAgentAllowanceIncrease(a.id),consentHash:digest({action:'fund-mainnet-agent-allowance',authority:a}),funding:{status:row?.deposit??'none',phase:this.running?this.phase:row?.deposit==='pending'?'needs_reconciliation':this.phase,error:this.error,transaction:proof?.transaction??null},
      ...
      custody:'Ledger approves Base mainnet USDC transfers to a dedicated software spending wallet. Every allowance increase requires another explicit Ledger signature. The trusted broker keeps services, recipients, per-call limits, rolling limits and expiry unchanged.',
  ...
  private async supported(){
    if(await this.chain.chainId()!==8453)throw new Error('Funding RPC is not Base mainnet');
    const supported=await this.facilitator.getSupported();
    if(!supported.kinds.some(k=>k.x402Version===2&&k.scheme==='exact'&&k.network===this.options.authority.network))throw new Error('Funding facilitator does not advertise exact Base mainnet');
  }
  private async createPayload(amountBaseUnits:string, description:string, onPhase:(phase:string)=>void){
    ...
    if(await this.chain.balance()<BigInt(amountBaseUnits))throw new Error('Ledger payer has insufficient Base mainnet USDC');
    ...
    const offer:PaymentRequirements={scheme:'exact',network:a.network,asset:a.asset,amount:amountBaseUnits,payTo:a.spendingAddress,maxTimeoutSeconds:300,extra:{name:'USD Coin',version:'2',assetTransferMethod:'eip3009'}};
  ...
  async fund(consentHash:string):Promise<Record<string,unknown>>{
    ...
    let exported=false;
    try{
      const created=await this.createPayload(a.ceilingBaseUnits,'Fund the reviewed Mandate mainnet spending wallet',phase=>this.phase=phase);
  ...
  reviewIncrease(amountBaseUnits:string){
    ...
    if(resulting>MAX_EFFECTIVE_ALLOWANCE_BASE_UNITS)throw new Error('The total allowance cannot exceed 5 USDC');
    ...
    return {...review,consentHash:digest({action:'increase-mainnet-agent-allowance',authorityHash:digest(a),...review})};
  }
  async increase(amountBaseUnits:string,consentHash:string):Promise<Record<string,unknown>>{
    ...
    try{
      const created=await this.createPayload(review.amountBaseUnits,'Increase the reviewed Mandate mainnet allowance',phase=>this.increasePhase=phase);
