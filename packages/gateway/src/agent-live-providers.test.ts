...
test('candidate review never converts repeated ratings or missing capability metadata into a trust score',()=>{
 const result=analyzeCandidateCoverage({agents:[{id:'84532:1',registrationFile:{name:'Example'},feedback:[{clientAddress:'0x1',value:'99',isRevoked:false},{clientAddress:'0x1',value:'1',isRevoked:false}],validations:[]}]},['84532:1','84532:2']);
 ...
 assert.deepEqual(result.missingIds,['84532:2']);assert.ok((candidate.evidenceGaps as string[]).length>=4);
});
test('Hedera analysis tool cannot accept arbitrary URLs, extra instructions or non-observed-format IDs',()=>{
 const tool=createAgentTools({protocols,testnetEndpoints:{'hedera-analysis':'http://127.0.0.1:8423/tools/hedera-analysis'}})[0]!;
