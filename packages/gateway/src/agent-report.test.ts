...
test('reports derive confirmed and reserved amounts from actual payment rows, not model prose',()=>{
 const journal=new Journal(':memory:');try{
  ...
  store.event(run.id,'tool_observation',{requestId:'paid',toolId:'graph-protocol',data:{fixture:true},sources:[{url:'https://example.org/source',title:'Source'},{url:'javascript:alert(1)',title:'Not a source'}],receipt:{network:'eip155:8453',asset:'USDC',amountBaseUnits:'1000',transaction:`0x${'ab'.repeat(32)}`}});
  ...
  const report=buildAgentReport(store.run(run.id)!,store.events(run.id),[
   {id:'paid',state:'accepted',network:'eip155:8453',asset:'USDC',amountBaseUnits:'1000',transaction:`0x${'ab'.repeat(32)}`,chainVerified:true,error:null,createdAt:1},
  ...
  const markdown=agentReportToMarkdown(report);assert.match(markdown,/0\.001 USDC/);assert.match(markdown,/0\.002 USDC/);assert.match(markdown,/Base|eip155:8453/);assert.doesNotMatch(markdown,/javascript:/);
 }finally{journal.close();}
});
test('explorer links reject retired testnet or an arbitrary supplied URL',()=>{assert.equal(receiptExplorer('eip155:84532',`0x${'ab'.repeat(32)}`),null);assert.equal(receiptExplorer('hedera:testnet','https://attacker.invalid'),null);assert.match(receiptExplorer('hedera:testnet','0.0.5-1789171200-000000001')!,/hashscan.io\/testnet/);assert.equal(formatTestUsdc('1'),'0.000001');assert.equal(formatTestUsdc('123456789'),'123.456789');});
