...
(() => {
  ...
  function allowanceFacts(root, setup, compact = false) {
    ...
    fact(root, 'Shared allowance', `${units(effective)} USDC on Base mainnet`);
    if (BigInt(effective) !== BigInt(a.ceilingBaseUnits)) { fact(root, 'Initial Ledger funding', `${units(a.ceilingBaseUnits)} USDC`); fact(root, 'Confirmed increases', `${units(setup.confirmedIncreaseBaseUnits || '0')} USDC`); }
    fact(root, 'Per-call limit', `${units(a.perCallBaseUnits)} USDC · unchanged by increases`);
  ...
  function renderSetup(data) {
    ...
    for (const section of document.querySelectorAll('[data-needs-allowance]')) section.hidden = !setup;
    if (!setup) {
  ...
  function receiptList(payments, running = false) {
    ...
    for (const payment of payments) {
      ...
      head.append(el('strong', `${units(payment.amountBaseUnits)} USDC`), badge(networkLabel(payment.network), 'neutral'), badge(readable(payment.state), tone(payment.state))); row.append(head);
  ...
  async function refreshRun() {
    ...
    detail.hidden = false; $('noSelectedRun').hidden=true; detail.replaceChildren();
    ...
    if (['partial','interrupted','failed'].includes(run.state)) detail.append(button('Resume from saved evidence', 'secondary', async () => {
      try { await api(`/api/agent-runs/${encodeURIComponent(run.id)}/resume`, {}); runLast=''; await refreshRun(); }
    ...
    if (!terminal.has(run.state)) {
      ...
      else if (latest?.kind === 'payment_requested') state = `Verifying ${units(latest.data.quote.amountBaseUnits)} USDC on ${networkLabel(latest.data.quote.network)} for ${toolLabels[latest.data.quote.toolId]}.`;
    ...
    if (report) {
      ...
      for (const [label, value] of [['Confirmed spending', `${units(report.paidBaseUnits)} USDC`], ['Reserved / uncertain', `${units(report.reservedBaseUnits)} USDC`], ['Evidence sources', String(report.evidence.length)]]) { const item = el('div'); item.append(el('span', label), el('strong', value)); metrics.append(item); }
    ...
    for (const event of events.filter(e => ['action_selected', 'tool_observation', 'unpaid_tool_failure', 'price_blocked', 'payment_uncertain'].includes(e.kind))) {
      ...
      else if (event.kind === 'tool_observation') row.append(el('p', `${toolLabels[event.data.toolId]} · ${event.data.error ? 'paid result needs attention' : 'evidence received'} · ${units(event.data.receipt.amountBaseUnits)} USDC`, 'subtle'));
      else row.append(el('p', event.data.reason || readable(event.kind), 'error'));
      timeline.append(row);
    }
    detail.append(timeline);
    if (report) {
      detail.append(button('View payment receipts', 'text-button', () => showDashboardView('evidence')));
    ...
    if(terminal.has(run.state)&&run.result){
      const follow=el('details',undefined,'report-followup');follow.dataset.group='followup';follow.open=openDetails.has('followup');follow.append(el('summary','Ask about this report'));
      const answers=el('div');for(const event of events.filter(e=>e.kind==='report_followup')){answers.append(el('strong',event.data.question),renderMarkdown(event.data.answer));}
      const form=el('form'),label=el('label','Your question'),input=el('textarea');input.rows=2;input.maxLength=2000;input.required=true;label.append(input);const submit=el('button','Ask about report','secondary');submit.type='submit';const status=el('p',undefined,'subtle');status.setAttribute('role','status');form.append(label,submit,status);
      form.onsubmit=async event=>{event.preventDefault();submit.disabled=true;status.textContent='Reading the saved evidence…';try{await api(`/api/agent-runs/${run.id}/followup`,{question:input.value});runLast='';await refreshRun();}catch(e){status.textContent=e.message;}finally{submit.disabled=false;}};
      follow.append(answers,form,el('small','Uses saved evidence only. No tool payments; model usage is billed separately.'));detail.append(follow);
    }
  ...
  function openFunding() {
    ...
    $('agentFundingReviewText').textContent = `Fund ${units(setup.authority.ceilingBaseUnits)} USDC on Base mainnet. This transfers real USDC.`;
  ...
  async function previewIncrease() {
    ...
    try {
      ...
      $('agentIncreaseReviewText').textContent = `Add ${units(review.amountBaseUnits)} USDC: shared allowance becomes ${units(review.resultingCeilingBaseUnits)} USDC.`;
      const facts = $('agentIncreaseReviewFacts'); facts.replaceChildren(); fact(facts, 'Current allowance', `${units(review.currentCeilingBaseUnits)} USDC`); fact(facts, 'Additional funding', `${units(review.amountBaseUnits)} USDC`); fact(facts, 'New allowance', `${units(review.resultingCeilingBaseUnits)} USDC`); fact(facts, 'Ledger payer', review.from); fact(facts, 'Funded wallet', review.to); fact(facts, 'Per-call stays', `${units(review.unchanged.perCallBaseUnits)} USDC`); fact(facts, 'Expiry stays', when(review.unchanged.expiresAt));
  ...
  $('agentIncreaseForm').onsubmit = async event => {
    ...
    try { await api('/api/agent-setup/increase', { amountBaseUnits, consentHash: increaseReview.consentHash, confirm: 'increase_mainnet_agent_allowance' }); $('agentIncreaseDialog').close(); message('Review the additional funding transfer on your physical Ledger. The allowance increases only after chain verification.'); last = ''; await refreshAgents(); }
  ...
  $('agentReviewReturn').onclick = async () => {
    ...
    try {
      ...
      $('agentReturnReviewText').textContent = review.amountBaseUnits === '0' ? 'The broker will verify that the fully spent wallet has no remaining USDC. No zero-value transfer will be created.' : `Return ${units(review.amountBaseUnits)} USDC on Base mainnet. This returns real USDC to the original Ledger payer.`;
      const facts = $('agentReturnReviewFacts'); facts.replaceChildren(); fact(facts, 'From', review.from); fact(facts, 'Original Ledger payer', review.to); fact(facts, 'Observed balance', `${units(review.amountBaseUnits)} USDC`); fact(facts, 'Source block', review.block);
  ...
  $('agentReturnForm').onsubmit = async event => {
    ...
    try { await api('/api/agent-setup/return', { consentHash: returnReview.consentHash, confirm: 'return_unused_usdc' }); $('agentReturnDialog').close(); last = ''; await refreshAgents(); message('The return was requested. The allowance closes only after the transfer is verified.'); }
