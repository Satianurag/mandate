...
| 7. Broken build and irreproducible verify | Actual type-contract repairs; lockfile install; deterministic checks separate from live/descriptor probes; CI definitions | Final Mac verification completed with exit 0: 146 tests, all four workspaces built. Clean Linux installation/runtime verification has its own log and exit record; final report records its result rather than treating the Dockerfile as proof. |
...
The current observed count is 146: 4 parser + 5 facilitator + 126 gateway +
6 mandate-service + 2 service + 3 resolver tests. Test fixtures remain isolated under
tests/test-support; the audit itself permits test doubles and they are never called
live payment evidence.

`npm run test:ui` uses a real local HTTP app and persistence. It checks desktop and
mobile layouts, authentication, exact limit persistence, stop across refresh,
keyboard focus/dialog dismissal, no page errors, and the automated WCAG A/AA scan.
...
device. The final run passed the same 146-test deterministic gate. It is a clean
build/test proof, not the headless paid-broker proof.
