...
if (config.probe) {
  ...
  for (const [path, body, expected] of [
    ...
    ['/api/tasks', { query: config.query, authorizeFunding: true }, 403],
    ['/api/tasks', { query: config.query, network: 'eip155:1' }, 400],
    ['/api/tasks', { query: '{ agents(first: 1) { id } }' }, 403],
  ]) {
...
const response = await call('/api/tasks', { query: config.query, requestId: config.requestId });
...
while (Date.now() < deadline) {
  ...
  if (task.state === 'succeeded') {
    const body = JSON.parse(task.result).data;
    send({ type: 'result', result: {
      ok: true, kind: 'deterministic-paid-agent-comparison', requestId: config.requestId,
